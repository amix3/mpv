...
A simple script that shows a pause indicator, on pause
    fork of  https://github.com/Samillion/ModernZ/tree/main/extras/pause-indicator-lite
    - added fading animation.
...
local options = {
    ...
    fade_animation = true,                -- enable/disable fading animation
    ...
    -- play icon
    triangle_width = 69,                  -- width of triangle
...
-- pause icon
local function draw_rectangles(alpha)
    if options.fluent_icons then
        ...
        return string.format([[{\\rDefault\\an5\\alpha&H%s\\bord%s\\1c&H%s&\\3c&H%s&\\fs%s\\fn%s}%s]],
            alpha, options.icon_border_width, icon_color, icon_border_color, options.fluent_icon_size, icon_font, pause_icon)
    else
        return string.format([[{\\rDefault\\p1\\an5\\alpha&H%s\\bord%s\\1c&H%s&\\3c&H%s&}m 0 0 l %d 0 l %d %d l 0 %d m %d 0 l %d 0 l %d %d l %d %d{\\p0}]],
            alpha, options.icon_border_width, icon_color, icon_border_color, options.rectangles_width, options.rectangles_width, 
...
-- play icon
local function draw_triangle(alpha)
    if options.fluent_icons then
        local play_icon = "\238\166\143"
        return string.format([[{\\rDefault\\an5\\alpha&H%s\\bord%s\\1c&H%s&\\3c&H%s&\\fs%s\\fn%s}%s]],
            alpha, options.icon_border_width, icon_color, icon_border_color, options.fluent_icon_size, icon_font, play_icon)
    else
        return string.format([[{\\rDefault\\p1\\an5\\alpha&H%s\\bord%s\\1c&H%s&\\3c&H%s&}m 0 0 l %d %d l 0 %d{\\p0}]],
            alpha, options.icon_border_width, icon_color, icon_border_color, options.triangle_width, options.triangle_height / 2, options.triangle_height)
    end
end
...
-- draw and update indicator with optional fading animation
local function update_indicator()
    local w, h, display_aspect = mp.get_osd_size()
    if display_aspect == 0 then return end

    local draw_func = options.indicator_icon == "play" and draw_triangle or draw_rectangles

    if options.fade_animation then
        local start_time = mp.get_time()
        local duration = options.indicator_timeout

        local timer
        timer = mp.add_periodic_timer(1/60, function()
            local elapsed = mp.get_time() - start_time
            local progress = math.min(elapsed / duration, 1)
            local alpha = string.format("%02X", math.floor(progress * 255)) -- Fade from opaque to transparent

            indicator.data = draw_func(alpha)
            indicator:update()
...
local function flash_icon()
    ...
    if options.fade_animation then
        ...
        timer = mp.add_periodic_timer(1/60, function()
            ...
            flash.data = draw_triangle(alpha)
            flash:update()
        ...
        flash.data = draw_triangle(icon_opacity)
        flash:update()
